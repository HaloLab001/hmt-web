 ...
 *
 * IDENTIFICATION
 *	  hmt-admin/src/main/java/com/wugui/hmt/admin/controller/AutomationController.java
 *
 *-------------------------------------------------------------------------
 */
@RestController
...
public class AutomationController extends BaseController {
    ...
    @PostMapping("/create")
    @ApiOperation("创建自动化迁移任务")
    public ReturnT<String> add(HttpServletRequest request, @RequestBody AutomationCreatePojo automationCreatePojo) {
        automationCreatePojo.setUserId(getCurrentUserId(request));
        return automationService.add(automationCreatePojo);
    }

    @PostMapping("/triggerStart")
    @ApiOperation("启动任务调度")
    public ReturnT<String> triggerStart(int id) {
        return automationService.triggerStart(id);
    }

    @PostMapping("/triggerStop")
    @ApiOperation("关闭任务调度")
    public ReturnT<String> triggerStop(int id) {
        return automationService.triggerStop(id);
    }
