 ...
 *
 * IDENTIFICATION
 *	  hmt-admin/src/main/java/com/wugui/hmt/admin/core/thread/JobRegistryMonitorHelper.java
 *
 *-------------------------------------------------------------------------
 */
		...
		registryThread = new Thread(() -> {
			while (!toStop) {
				try {
					...
					if (!toStop) {
						logger.error(">>>>>>>>>>> hmt-web, job registry monitor thread error:{}", e);
					}
				}
				try {
			...
			logger.info(">>>>>>>>>>> hmt-web, job registry monitor thread stop");
		});
		registryThread.setDaemon(true);
		registryThread.setName("hmt-web, admin JobRegistryMonitorHelper");
