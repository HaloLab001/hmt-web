...
import com.wugui.hmt.admin.service.JobDataContrastService;
...
public class JobDataContrastController {
    ...
    @PostMapping("/createDataContrastTask")
    @ApiOperation("创建数据对比任务")
    public ReturnT<String> createDataContrastTask(@RequestBody JobDataContrast jobDataContrast) {
        return jobDataContrastService.createDataContrastTask(jobDataContrast);
    }

    @PostMapping("/createStructureContrastTask")
    @ApiOperation("创建结构对比任务")
    public ReturnT<String> createStructureContrastTask(@RequestBody JobDataContrast jobDataContrast) {
        return jobDataContrastService.createStructureContrastTask(jobDataContrast);
    }

    @GetMapping("/pageList")
    @ApiOperation("数据对比任务列表")
    public ReturnT<Map<String, Object>> pageList(@RequestParam(required = false, defaultValue = "0") int current,
        ...
        return new ReturnT<>(jobDataContrastService.pageList((current-1)*size, size, taskName, metaType));
    }

    @RequestMapping(value = "/remove", method = RequestMethod.POST)
    @ApiOperation("删除数据对比任务")
    public ReturnT<String> remove(int id) {
        return jobDataContrastService.delete(id);
    }

    @GetMapping("/pageDetailsList")
    @ApiOperation("数据对比任务详情列表")
    public ReturnT<Map<String, Object>> pageDetailsList(@RequestParam(required = false, defaultValue = "0") int current,
                                                 @RequestParam(required = false, defaultValue = "10") int size,
                                                 int taskId, String tableName, int isCorrect, int fastQue, String metaTypeId) {
        return new ReturnT<>(jobDataContrastService.pageDetailsList((current-1)*size, size, taskId, tableName, isCorrect, fastQue, metaTypeId));
    }

    @GetMapping("/detailsCount")
    @ApiOperation("数据对比任务详情统计")
    public ReturnT<Map<String, Object>> detailsCount(int taskId, String metaTypeId) {
        return new ReturnT<>(jobDataContrastService.detailsCount(taskId, metaTypeId));
    }
